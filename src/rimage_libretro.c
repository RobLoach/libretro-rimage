...
bool InitCore() {
  ...
  bushy = LoadImage("grafxkid-sprite.png");
...
void UpdateGame() {
  ...
  if (bushy.data != NULL) {
    Rectangle srcRec = {0, 0, bushy.width, bushy.height};
    Rectangle dstRec = {40, 80, bushy.width, bushy.height};
    ImageDraw(&core->backBuffer, bushy, srcRec, dstRec, WHITE);
  }
}
